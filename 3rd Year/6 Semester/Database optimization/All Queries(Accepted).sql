...
INSERT INTO Course (Name, Description, startingDate, EndingDate, CategoryID) VALUES
('Mathematics III', 'High level math course', '2025-03-10', '2025-07-13',
	(SELECT CategoryID FROM Category WHERE Name='Mathematics'));

INSERT INTO Course (Name, Description, startingDate, EndingDate, CategoryID) VALUES
('Differential Equestions', 'Diff Equestion math course', '2021-03-14', '2023-07-13',
	(SELECT CategoryID FROM Category WHERE Name='Mathematics'));
...
INSERT INTO Course_Dependenc (CourseID, DependsOnCourseID, Description)
VALUES (
    (SELECT CourseID FROM Course WHERE Name='Mathematics III'),
    (SELECT CourseID FROM Course WHERE Name='Mathematics II'),
    'Requires understanding of mathematics I'
);

INSERT INTO Course_Dependenc (CourseID, DependsOnCourseID, Description)
VALUES (
    (SELECT CourseID FROM Course WHERE Name='Differential Equestions'),
    (SELECT CourseID FROM Course WHERE Name='Mathematics III'),
    'Requires understanding of mathematics III'
);
...
GROUP BY S.StudentID, S.FirstName, S.SecondName, S.NikeName
HAVING COUNT(C.CourseID) - COUNT(CASE WHEN SP.Marks >= 3 THEN 1 END) = 1
ORDER BY S.SecondName, S.FirstName;
----------------------------------------------------------------------------------------
...
WHERE EXISTS (
    ...
    EXCEPT
    SELECT StudentID FROM Student_Plan
);
----------------------------------------------------------------------------------------
...
WHERE SP.Planned_Finish >= CURRENT_DATE
  AND EXISTS (
    SELECT 1
    FROM StudentPerformance SP2
    WHERE SP2.CourseID = SP.CourseID
      AND SP2.marks >= 3
    GROUP BY SP2.CourseID
    HAVING COUNT(DISTINCT SP2.StudentID) >= 2
)
...
SELECT 
    ...
    C2.CourseID AS Course2ID,
    C2.Name AS Course2_name,
    COUNT(DISTINCT CT1.ThemeID) AS CommonTopic
FROM Course C1
JOIN Course C2 ON C1.CourseID < C2.CourseID  -- For avoiding the duplication(A-B 0r B-A)
...
ORDER BY CommonTopic DESC, C1.Name, C2.Name;
-----
WITH CoursePairs AS (
    SELECT 
        CT1.CourseID AS Course1ID,
        CT2.CourseID AS Course2ID,
        CT1.ThemeID
    FROM CourseTheme CT1
    JOIN CourseTheme CT2 ON CT1.ThemeID = CT2.ThemeID
    WHERE CT1.CourseID < CT2.CourseID
)
SELECT 
    CP.Course1ID,
    ...
    COUNT(DISTINCT CP.ThemeID) AS CommonTopics
FROM CoursePairs CP
JOIN Course C1 ON CP.Course1ID = C1.CourseID
JOIN Course C2 ON CP.Course2ID = C2.CourseID
GROUP BY CP.Course1ID, C1.Name, CP.Course2ID, C2.Name
HAVING COUNT(DISTINCT CP.ThemeID) >= 2
ORDER BY CommonTopics DESC, C1.Name, C2.Name;
----------------------------------------------------------------------------------------
		-- 39. Выбрать тройки однофамильцев по всей БД. (for teachers and the students and the teacher&student through the CTE)
WITH Surnames AS (
    SELECT Secondname
    FROM Student
    GROUP BY Secondname
...
WITH MyLastNames AS (
    SELECT 
        ...
        S.FirstName,
        S.SecondName AS LastName,
        S.NikeName AS MiddleName
    FROM Student S
    UNION ALL
    SELECT 
...
SELECT 
    ...
    ML.MiddleName
FROM MyLastNames ML
    JOIN Surnames SN ON ML.LastName = SN.LastName
...
SELECT *
FROM CourseAverages
WHERE AvgMark = (SELECT MAX(AvgMark) FROM CourseAverages);

----------------------------------------------------------------------------------------
		--43. Выбрать самую маленькую и самую большую аудитории по количеству мест.
SELECT MIN(Capacity), MAX(Capacity)
FROM Classroom;
----------------------------------------------------------------------------------------
		--44. Выбрать для каждого преподавателя количество курсов, которые он преподает,
		-- и общее количество курсов, которые есть в БД.
SELECT 
    T.teacherid,
    T.lastname,
    T.firstname,
...
ORDER BY Amount_courses_teacher DESC, T.lastname;
--Also through the okon function. 

--==================================================================================================
WITH TeacherCourseCount AS (
    SELECT 
        T.teacherid,
        T.lastname,
        T.firstname,
        T.middlename,
        COUNT(DISTINCT CT.courseid) AS teacher_course_count
    FROM Teacher T
        LEFT JOIN Lesson L ON T.teacherid = L.teacherid
        LEFT JOIN CourseTheme CT ON L.themeid = CT.themeid
    GROUP BY T.teacherid, T.lastname, T.firstname, T.middlename
...
SELECT 
    ...
    firstname,
    middlename,
    teacher_course_count AS amount_courses_teacher,
    SUM(teacher_course_count) OVER () AS whole_amount_courses_db thorugh the partiction
FROM TeacherCourseCount
ORDER BY amount_courses_teacher DESC, lastname; 
---thoruhg the partion "COUNT(DISTINCT CT.courseid)" in the
--" SUM(teacher_course_count) OVER () AS whole_amount_courses_db thorugh the partiction" 
-- and "over" is not necessary here, when we want to use the it. 
--==================================================================================================
--====================== The new one =================
SELECT 
    teacherid,
...
----------------------------------------------------------------------------------------
		-- 46. Выбрать названия курсов, которые нужно пройти до курса N 
		-- (значение подставьте сами). Учесть, что такой курс может быть не один.
		
SELECT 
    C.Name ,
    C.CourseID
FROM Course_Dependenc CD
    JOIN Course C ON CD.DependsOnCourseID = C.CourseID
WHERE CD.CourseID = 5
ORDER BY C.Name;
-------------------------------------
SELECT 
...
--through recursion, 
--=========================================================================================
---new one in recursion
WITH RECURSIVE Prerequisites AS (
    SELECT 
        C.CourseID,
        C.Name,
        1 AS level,
        CAST(C.Name AS VARCHAR(500)) AS path
    FROM Course C
    WHERE NOT EXISTS (
        SELECT 1 FROM Course_Dependenc CD 
    ...
    SELECT 
        ...
        C.Name,
        P.level + 1,
        CAST(P.path || ' -> ' || C.Name AS VARCHAR(500))
    FROM Prerequisites P
    JOIN Course_Dependenc CD ON P.CourseID = CD.DependsOnCourseID
...
WHERE 
    ...
    -- НО не сдал предшествующий курс
    AND NOT EXISTS (
        SELECT 1
        FROM StudentPerformance SP2
...
---------------------------------------------------------------------------------
			-- 48. Выбрать самую длинную цепочку зависимостей курсов.
---Here we should named all the courses that are this chain
WITH RECURSIVE CourseChain AS (
    SELECT 
        C.CourseID,
        C.Name,
        1 AS chain_length
    ...
    SELECT 
        ...
        CC.chain_length + 1
    FROM CourseChain CC
    JOIN Course_Dependenc CD ON CC.CourseID = CD.CourseID
    JOIN Course C ON CD.DependsOnCourseID = C.CourseID
)
SELECT MAX(chain_length) AS Logest_Chain
FROM CourseChain;
-------------
--we should print names of all  courses which they are exist in the chain. 
WITH RECURSIVE CourseChain AS (
    SELECT 
        C.CourseID,
        C.Name,
        1 AS chain_length,
        CAST(C.Name AS VARCHAR(500)) AS chain_path
    FROM Course C
    UNION ALL
    SELECT 
        C.CourseID,
        C.Name,
        CC.chain_length + 1,
        CAST(C.Name || ' -> ' || CC.chain_path AS VARCHAR(500))
    FROM CourseChain CC
    JOIN Course_Dependenc CD ON CC.CourseID = CD.CourseID
    JOIN Course C ON CD.DependsOnCourseID = C.CourseID
)
SELECT 
    chain_path AS biggest_chain,
