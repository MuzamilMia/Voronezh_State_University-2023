...
int find_Max_depth(Tree root)
{
	int result{};
	if (root)
	{
		int left_result = find_Max_depth(root->left);
		int right_result = find_Max_depth(root->right);
...
int Depth_of_tree(Tree root)
{
	int result{ -1 };
	if (root)
	{
		int leftMax = Depth_of_tree(root->left);
		int rightMax = Depth_of_tree(root->right);

		result= std::max(leftMax, rightMax) + 1;
	}
	return result;
}
int task_stack(Tree root)
{
	Tree t = root;
	int max_depth{};
...
int main()
{
	...
	if (file)
	{
		...
		std::cout << "The Max depth of the tree by recursion is: " << Depth_of_tree(root);
		std::cout << "\nThe Max depth of the tree by stack is: " << task_stack(root);
